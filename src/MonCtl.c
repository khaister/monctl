            ...
            switch (propToken[0]) {
                case 'i': //id
                    ...
                    while ((idToken = strtok_r(propToken, "+", &propToken))) {
                        ...
                        if (j > 127) {
                            fprintf(stderr, "Current code only supports 128 screens mirroring. Please open a GitHub issue at https://github.com/khaister/monctl if you need this raised.\n");
                        }
                    }
...
void printHelp() {
    printf(
            ...
            "    `monctl list` output changed slightly in v1.4.0. If this broke your scripts, use `monctl list --v1.3.0`.\n"
            "\n"
            "Feedback:\n"
...
void printCurrentProfile() {
    ...
    printf("Execute the command below to set your screens to the current arrangement. If screen ids are switching, please run `monctl --help` for info on using contextual or serial ids instead of persistent ids.\n\n");
    printf("monctl");
    for (int i = 0; i < screenCount; i++) {
...
bool setPositions(ScreenConfig* screenConfigs, int argc, CGDisplayConfigRef configRef, CGDirectDisplayID screenList[], CGDisplayCount screenCount) {
    ...
    for (int i = 0; i < argc - 1; i++) {
        ...
        if (((int) curOrigin.x != screenConfigs[i].x || (int) curOrigin.y != screenConfigs[i].y) && screenCount > 1) { //setting a screen to its current origin makes monctl hang for a couple seconds. If there is only one screen, macOS will force the origin to be (0,0) so we do not need to set it.
